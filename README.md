...
- [Agent trust summary](#agent-trust-summary)
...
4. **Daydreams Extension (`@dreamkeeper/core`)** — A native Daydreams module exposing 12 KeeperHub-backed actions across **two live networks** (Base Sepolia and KeeperHub's Tempo testnet): transfers, arbitrary contract calls, atomic check-and-execute, curated protocol actions, a sign-hold-release/cancel payment lifecycle, reconciliation, audit, spending-limit reads, and a self-reported trust summary — see [KeeperHub endpoints integrated](#keeperhub-endpoints-integrated) below.
...
| _(none — local only)_         | `keeperhub_get_trust_summary`                                                | Reads this agent's own self-reported firewall-block/execution-outcome counters and circuit breaker state | A local, locally-persisted record an agent exposes about its own behavior — **not** a hosted or cross-agent-queryable trust score today; see [Agent trust summary](#agent-trust-summary) below for that scope boundary                                                                        |
...
Every firewall block (by reason) and every real execution outcome (confirmed/failed/indeterminate) is counted in a small persisted ledger, readable via `keeperhub_get_trust_summary` alongside the circuit breaker's current state — e.g. `{ totalBlockedAttempts: 4, blockedByReason: { RECIPIENT_NOT_WHITELISTED: 3, AMOUNT_EXCEEDS_TX_CAP: 1 }, totalConfirmedExecutions: 12, totalFailedExecutions: 0, totalUnknownExecutions: 0, circuitBreakerState: "CLOSED" }`. Survives a process restart the same way the circuit breaker and idempotency store do, when `persistDir` is set (see [`packages/core/src/firewall/trust-ledger.ts`](packages/core/src/firewall/trust-ledger.ts)).

**What this is not, stated plainly**: this is a *self-reported, locally-persisted* counter set — not a hosted service, not cryptographically attested, and not reachable by a counterparty unless it has direct access to this same running instance. A real cross-agent trust registry (another agent checking a stranger's history before transacting with it) would mean building and hosting a separate service — auth, identity, uptime — from scratch. There is no KeeperHub-native endpoint for this (checked directly against `search_protocol_actions`; the only adjacent tool, `blockscout/get-address-info`, reports a wallet address's on-chain reputation via a third party, not an agent's own firewall/execution history). That's out of scope here — this ledger is the local primitive a future hosted version would be built on top of, not that version itself.
...
| [`packages/core/src/daydreams/actions.ts`](packages/core/src/daydreams/actions.ts)                     | Defines all 12 Daydreams action handlers (`keeperhub_*`), each wrapping one `KeeperHubClient` method with a typed Zod schema                                                   |
...
- **Gas-Price Ceiling Only Covers the Direct-Signer Fallback.** `FirewallPolicy.maxGasPriceGwei` refuses a dry-run when the on-chain fallback's own RPC gas price exceeds the ceiling — but the real KeeperHub execution path has no equivalent check, since KeeperHub reprices gas server-side before this code ever sees a price to compare.
...
- **The Trust Summary Is Local, Not a Hosted Cross-Agent Registry.** `keeperhub_get_trust_summary` counts this agent's own firewall blocks and execution outcomes, persisted the same way as the circuit breaker — but a counterparty can't check it unless it has direct access to this same running instance. There's no KeeperHub-native endpoint for hosting or serving it (checked directly; the only adjacent tool reports a wallet address's on-chain reputation via a third party, not an agent's own history), and a real cross-agent version would mean building and hosting a separate service from scratch — out of scope here.

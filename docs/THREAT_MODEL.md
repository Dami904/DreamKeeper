...
### Threat 6: Hallucinated or Adversarial Tempo `paymentId`

- **Attack Vector**: Unlike the transfer/contract-call/check-and-execute paths, a Tempo hold has no client-side `DryRunToken`/intent-hash binding — KeeperHub itself holds the signed artifact server-side once `keeperhub_tempo_sign_and_hold` succeeds. Without a guard, a prompt-injected or hallucinating agent could invent, or be fed, an arbitrary `paymentId` string and pass it to `keeperhub_tempo_release_hold`/`keeperhub_tempo_cancel_hold` — and if it happened to match a real hold the organization owns (created by this process or another), KeeperHub would honor the call.
- **Mitigation**: `KeeperHubClient` tracks every `paymentId` it creates via `tempoSignAndHold()` in an in-memory map (`activeTempoHolds`) and refuses `tempoReleaseHold()`/`tempoCancelHold()` for any `paymentId` not in that map, returning `TEMPO_PAYMENT_ID_UNKNOWN` before ever calling the transport — the same "never trust an LLM-supplied identifier blindly" posture used elsewhere (dry-run token IDs, intent-hash binding). This tracking is in-memory only (see `docs/LIMITATIONS.md`), so it does not survive a process restart.
- **Secondary mitigation, independent of this repo**: Tempo's own protocol enforces an on-chain `validBefore` expiry on every signed hold — per KeeperHub's tool description, "the on-chain validity window guarantees it can never settle after its deadline." This is a KeeperHub/Tempo-native staleness guard distinct from DreamKeeper's own 60-second `DryRunToken` TTL, and it applies even if this repo's own tracking were somehow bypassed.
